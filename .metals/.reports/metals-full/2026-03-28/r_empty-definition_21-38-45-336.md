error id: file://<WORKSPACE>/doto-api/app/controllers/AuthController.scala:async.
file://<WORKSPACE>/doto-api/app/controllers/AuthController.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -io/circe/syntax/Action.async.
	 -io/circe/syntax/Action.async#
	 -io/circe/syntax/Action.async().
	 -play/api/mvc/Action.async.
	 -play/api/mvc/Action.async#
	 -play/api/mvc/Action.async().
	 -JsonHelper.Action.async.
	 -JsonHelper.Action.async#
	 -JsonHelper.Action.async().
	 -Action.async.
	 -Action.async#
	 -Action.async().
	 -scala/Predef.Action.async.
	 -scala/Predef.Action.async#
	 -scala/Predef.Action.async().
offset: 713
...
class AuthController @Inject()(
  ...
  def register: Action[AnyContent] = Action.asyn@@c { request =>
    val body = readBody(request)
    decode[RegisterRequest](body) match
      case Left(_)    => Future.successful(badRequest("Invalid request body"))
      case Right(req) =>
        if req.username.length < 3 || req.username.length > 50 then
